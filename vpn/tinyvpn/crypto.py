"""Cryptography, anti-replay, and Noise-style handshake helpers."""
...
def fingerprint(public_key_raw: bytes) -> str:
    return hkdf(public_key_raw, info=b"tinyvpn-fingerprint", length=8).hex()


class ReplayWindow:
    """2048-bit sliding bitmap used for anti-replay."""
...
class SecureChannel:
    """AES-256-GCM channel with explicit sequence numbers."""
    ...
    def decrypt(self, sequence: int, ciphertext: bytes, aad: bytes, replay: ReplayWindow) -> bytes | None:
        if not replay.check_and_update(sequence):
            return None
        nonce = b"\x00\x00\x00\x00" + struct.pack("!Q", sequence)
        try:
...
def respond_handshake_init(
    ...
    *,
    ...
    if len(packet) < 1 + 4 + 32 + 32 + 4 + 2 + 16:
        return None
    ...
    # Four Noise DH values in canonical order (server side):
    # 1. server_static x client_eph
    # 2. server_eph    x client_eph
    # 3. server_static x client_static
    # 4. server_eph    x client_static
    master = hkdf(
        local_static.exchange(initiator_eph)
...
def complete_handshake(
    ...
    *,
    ...
    if len(packet) < 1 + 4 + 32 + 4 + 2 + 16 or packet[0] != HANDSHAKE_REPLY:
        return None
    session_id = struct.unpack("!I", packet[1:5])[0]
    ...
    # Four Noise DH values in canonical order (client side):
    # 1. client_eph    x server_static  (= server_static x client_eph)
    # 2. client_eph    x server_eph     (= server_eph    x client_eph)
    # 3. client_static x server_static  (= server_static x client_static)
    # 4. client_static x server_eph     (= server_eph    x client_static)
    master = hkdf(
    ...
    init_to_resp, resp_to_init = master[:32], master[32:64]
    reply_key = hkdf(master, info=b"tinyvpn-noise-reply")
    # Retry with the actual tunnel id inside the decrypted payload once we know it.
    try:
